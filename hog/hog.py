...
def next_perfect_square(score):
    return (int(sqrt(score)) + 1) * (int(sqrt(score)) + 1)
# END PROBLEM 4
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    max_score, dice_num = 0, 0
    for i in range(1, 11):
        score_average = make_averaged(roll_dice, total_samples)
        average_num = score_average(i, dice)
        if average_num > max_score:
            max_score = average_num
            dice_num = i
    return dice_num
    # END PROBLEM 9
