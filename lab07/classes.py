...
class TutorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        if len(player.hand) > 0:
            player.hand.append(Card(player.hand[0].name, player.hand[0].attack, player.hand[0].defense))
            added = True
        if added:
...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if len(player.hand) > 0:
            ...
            for card0 in player.hand:
                if card0.power(opponent_card) > best_card.power(opponent_card):
                    best_card = card0
