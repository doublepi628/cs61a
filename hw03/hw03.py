...
def pingpong(n):
    ...
    if n == 1:
        ...
        return pingpong(n - 1) + pingpong_next(n)
...
def count_coins(change, up_limit = 25):
    ...
    if change == 0:
        return 1
    elif change >= up_limit:
        if next_smaller_coin(up_limit) != None:
            return count_coins(change - up_limit, up_limit) + count_coins(change, next_smaller_coin(up_limit))
        else:
            return count_coins(change - up_limit, up_limit)
    else:
        return count_coins(change, next_smaller_coin(up_limit))
